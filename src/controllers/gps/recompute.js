...
export const recomputeController = async (req, res) => {
  ...
  try {
    /* delete old trips collection before create new one */
    const tripsCollections = semutDB.db('trips').collection(imei.toString())
    await tripsCollections.deleteMany({ date })
    /* finish delete */
    ...
    const trips = filter_no_start_or_end_timestamp.map((data, index) => {
      ...
      // Check if fuel was refilled
      const isRefuel = fuel[0] < fuel[fuel.length - 1]
